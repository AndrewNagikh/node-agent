...
**Cluster**: 3 heterogeneous consumer machines on home LAN/Wi-Fi —
...
this project's showing stands or falls on. Everything here cites its
source report; nothing is hand-estimated. Where a number is thin (small
sample, one-off measurement), that's stated, not hidden.
...
Cited, not run on this cluster — two concrete blockers found before
attempting setup: prima.cpp doesn't support Windows (node-c is Windows),
and its supported quantizations (Q4K/Q6K/Q80/IQ1) don't overlap with what
this project uses (Q3_K). Their published number for a 70B model: ~2
tok/s (674ms/token TPOT) on a 4-device cluster with weaker GPUs than this
project's node-c, deliberately memory-constrained (37GiB combined
RAM+VRAM, by their own account insufficient for the full quantized
model) to exercise their disk-offload mechanism. This project's 2.28
tok/s on 3 devices with comfortable memory headroom is in the same
ballpark but not a controlled comparison — full reasoning in the
...
| Gate | Status |
|---|---|
| G0 Stranger test | Not attempted |
...
| G5 Reproducibility | Not started |
