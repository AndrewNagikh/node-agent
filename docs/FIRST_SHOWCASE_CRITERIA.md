...
The showable artifact is two things together:
...
G0 (stranger test), G5 (reproducibility/quickstart), and G7
(five-minute explanation) removed 2026-07-24 -- no second person
available to invite, which was the load-bearing requirement for all
three (a stranger to test with, a cold reader for the quickstart claim,
an audience to validate the explanation against). Not tracked as
"dropped" rows -- gone from this file entirely; per this doc's own rule
(line 11-12), anything not listed here isn't required.
...
| G3 | **Demo-grade reliability** | A 30-minute soak: >=20 sequential create/generate/destroy cycles across >=3 models (incl. one 32B+ rung) with zero manual node restarts. NOT fault tolerance -- just "doesn't fall over while someone watches". | **PASSED 2026-07-23** (docs/bench/2026-07-23_g3_soak/): run 3, 190/190 cycles, 100.0% success, zero crashes/corruption. Took 2 fixes: orchestrator's sequential no-retry coverage poll (689a554f6, run1 71%->run2 81%) then node_agent's /installed-layers full-checksum-reverify-on-every-poll (47990afaf, verify-result cache TTL 300s, run2 81%->run3 100%). Driven via the orchestrator HTTP API -- the same calls the dashboard makes. A separate dashboard-UI re-run was dropped 2026-07-24: it would exercise the button wiring, not the runtime this gate is about. |
| ~~G4~~ | ~~Network-aware placement (Task 21.1)~~ | ~~Entry role never lands on the worst-RTT node when an alternative fits.~~ | **REVERTED 2026-07-24** (`65ed1f60a`). Implemented 2026-07-22 (`b9a7dc1`/`6ef7bfe`), then removed: the planned A/B (TASK_21 Item 1) was never run, so the latency win stayed unmeasured, while the cost was real -- noisy home-network RTT moved the entry node between sessions, orphaning installed layers on the previous node (DEGRADED coverage, "runtime coverage not ready"; five models needed manual repair on 2026-07-24). Placement stability beats an unquantified few-ms gain. Entry is back to second-strongest-by-score, which is stable across sessions. |
...
- **Former G0/G5/G7 (stranger test, reproducibility, five-minute
  explanation), removed 2026-07-24.** All three needed a real second
  person -- a stranger to test with, a cold reader for the quickstart
  timing claim, an audience for the explanation -- and none is
  available. Revisit only if that changes. The underlying bench
  scripts (G5's other half) already exist and are used throughout
  this project's reports, so nothing substantive is lost.
...
6. **Report (G6, done)**, publish. (G0/G5/G7 removed 2026-07-24, G4
   reverted -- see §4.) **Nothing is outstanding: every remaining gate
   is passed.**
...
- **Where:** r/LocalLLaMA and/or HN with the report, repo public. If
  the first post sinks without engagement, try at least one more
  channel/timing before reading anything into it.
