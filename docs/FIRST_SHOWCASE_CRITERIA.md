...
| G1 | **Capacity proof on models that fit no single node** | 32B dense measured (runs today at 7.0-7.5 tok/s; compute its ceiling % for the report) AND the L2-MoE rung (qwen3-30b, catalog) measured -- the "feels fast" demo number. Both: cold sync, warm session, 64-token generation, >=80% of computed ceiling. | 32B runs, ceiling % not computed; 30B-MoE not attempted. |
...
| G3 | **Demo-grade reliability** | A 30-minute soak: >=20 sequential create/generate/destroy cycles across >=3 models (incl. one 32B+ rung) with zero manual node restarts, driven through the dashboard. NOT fault tolerance -- just "doesn't fall over while someone watches". | **2 runs 2026-07-23** (docs/bench/2026-07-23_g3_soak/): run 1 71% success (107 cycles), fixed orchestrator's sequential no-retry coverage poll (689a554f6) -> run 2 81% success (90 cycles). Improved but still FAILS gate. Deeper cause found: `/installed-layers` re-reads+re-hashes full tensor data on every poll (node_agent.cpp:3249) -- cheap-metadata fix not yet applied; qwen2.5-32b (most data) barely moved between runs, confirming this as the residual driver. |
...
| G6 | **The honest benchmark report** | One document: per-rung table (3B / 14B / 32B / 30B-MoE, plus Tier 2 rungs if reached) with tok/s, % of computed ceiling, spec on/off where a pair exists, TTFT, sync time; **prima.cpp on the same cluster** at the rungs actually achieved; limitations stated plainly: single stream, no fault tolerance, LAN-only, slow long-prompt prefill on big models. | Not started; methodology exists (bench protocol + PERFORMANCE_METRICS_SPEC). |
