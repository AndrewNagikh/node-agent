...
- Orchestrator (192.0.2.10) runs without `DIST_PERF_TRACE=1` → per-wave traces need manual `begin_decode` fanout to nodes (workaround used tonight), or fix item 1 above.
...
1. Update + rebuild + restart the orchestrator only (192.0.2.10) — nodes are already current.
