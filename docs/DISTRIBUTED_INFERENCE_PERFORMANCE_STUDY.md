...
> **Post-study resolution (Task 17.1, 2026-07-13):** the Docker bubble root cause was **TCP Nagle + delayed-ACK** — the transport never set `TCP_NODELAY`, and the small-message request/response ping-pong hit the ~40 ms delayed-ACK timer (twice per token under v2, matching the measured 40.8 ms v1 bubble and 82 ms v2 period exactly). With `TCP_NODELAY` set: Docker 11.4 → **60.7 tok/s**, bubble 71% → **2.5%** (RFC-0013 §28 gate PASS). The homelab 11.7 ms bubble must be re-measured with this fix before attributing anything to scheduler logic. See `TASK_17_1_ORCHESTRATOR_BUBBLE_CLOSURE.md`.
